...
\begin{align}
    \smoothmax(a, b) = \begin{dcases}
        ...
        a + \frac{X}{2k} \left( \frac{1}{1+e^{-kX}} + \frac{1}{1-e^{-kX}} \right) &\text{for } a \neq b \\
        a + \frac{1}{2k} &\text{for } a = b
    \end{dcases}
\end{align} 
with $X=b-a$ for conciseness, and $k$ a sharpness parameter approximating the $\max$ operator as $k$ grows. At $k=5$, the operator $\max$ is already well approximated while conserving continuousness in the resulting height field $\height(p) = \smoothmax(\heightSubsid(\p), \heightCoral(\p))$.
