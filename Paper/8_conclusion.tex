...
\paragraph{Abstraction of user control} Another limitation of using cGAN in this context is the lack of control over the initial parameters (height function, wind and wave deformations, and resistance function) during terrain generation. While traditional procedural generation methods allow users to adjust parameters during the generation process, the cGAN model operation is abstracted from the user, providing no mechanism for direct interaction beyond the initial label map. This reduces the level of customisation available to the user.
